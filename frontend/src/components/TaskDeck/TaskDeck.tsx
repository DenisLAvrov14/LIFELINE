...
const TaskDeck: React.FC<Props> = (props) => {
    ...
    const handlePauseOrResume = useCallback(async () => {
        if (isRunning) {
            // Pause logic
            try {
                const currentElapsed = Math.floor((Date.now() - (startTime?.getTime() || 0)) / 1000) + elapsedTime;
    ...
    const handleStopAndMarkAsDone = async () => {
        try {
            ...
            const duration = startTime
                ? Math.round((endTime.getTime() - startTime.getTime()) / 1000) + elapsedTime
                : elapsedTime;
    ...
    useEffect(() => {
        ...
        if (isRunning && startTime) {
            interval = setInterval(() => {
                const elapsed = Math.floor((Date.now() - startTime.getTime()) / 1000) + elapsedTime;
                setTime(elapsed);
            }, 1000);
        }
